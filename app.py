...
if page == "🏠 Dashboard":
    ...
    **Clear exits**: GREENPOWER, JAYSREETEA, SIYARAM-M → free ≈ ₹4.08 Lakh for redeployment.
    ...
    st.markdown("---")
    st.subheader("Live Prices (Yahoo Finance)")
    st.caption("Delayed quotes · cached ~5 min. All 52 may take 1–2 minutes.")
    scope = st.radio(
        "Scope",
        ["Top 10 by invested", "All 52 holdings"],
        horizontal=True,
        key="dash_live_scope",
    )
    if st.button("🔄 Fetch live prices", type="primary", key="dash_fetch_live"):
        if scope == "Top 10 by invested":
            symbols = df.nlargest(10, "invested")["symbol"].tolist()
        else:
            symbols = df.sort_values("invested", ascending=False)["symbol"].tolist()
        with st.spinner(f"Fetching live prices for {len(symbols)} stocks..."):
            prices = get_batch_live_prices(tuple(symbols))
        rows = []
        total_live_val = 0.0
        total_invested = 0.0
        n_ok = 0
        ...
        if n_ok > 0:
            live_pnl_all = total_live_val - total_invested
            c1, c2, c3 = st.columns(3)
            c1.metric("Live value (fetched)", f"₹{total_live_val:,.0f}")
            c2.metric("Invested (same set)", f"₹{total_invested:,.0f}")
            c3.metric("Live P&L", f"₹{live_pnl_all:,.0f}", f"{(live_pnl_all/total_invested)*100:+.1f}%")
        st.caption(f"Fetched {n_ok}/{len(symbols)} prices from Yahoo Finance. Micro-caps may return N/A.")
...
""")
    ...
    report_mode = st.radio(
        "Report scope",
        ["Single stock", "All 52 holdings (summary)"],
        horizontal=True,
        key="report_mode",
    )

    if report_mode == "All 52 holdings (summary)":
        st.subheader("All 52 holdings — summary report")
        fetch_live_all = st.checkbox("Include live prices (slower)", value=False, key="report_all_live")
        fetch_hl = st.checkbox("Include 1Y high/low (slower)", value=False, key="report_all_hl")
        ...
        if fetch_live_all:
            ...
            live_map = get_batch_live_prices(tuple(symbols))

        for i, sym in enumerate(symbols):
            status.text(f"Building row {i+1}/{n}: {sym}")
            r = df[df.symbol == sym].iloc[0]
            row = {
                "Symbol": sym,
                "Sector": r.sector,
            ...
            if fetch_live_all:
                ...
                if lv and lv.get("price"):
                    ...
                    val = px * r.qty
                    pnl = val - r.invested
                    row["Live Price"] = round(px, 2)
                    row["Live Value"] = round(val)
                    row["Live P&L %"] = round(pnl / r.invested * 100, 1)
                else:
                    row["Live Price"] = None
                    row["Live Value"] = None
                    row["Live P&L %"] = None
            if fetch_hl:
                h = get_history(sym, "1y")
                if h is not None and not h.empty:
                    row["1Y High"] = round(float(h["High"].max()), 1)
                    row["1Y Low"] = round(float(h["Low"].min()), 1)
                    last = float(h["Close"].iloc[-1])
                    row["1Y from High %"] = round((last / float(h["High"].max()) - 1) * 100, 1)
                else:
                    row["1Y High"] = None
                    row["1Y Low"] = None
                    row["1Y from High %"] = None
            rows.append(row)
            progress.progress((i + 1) / n)
        progress.empty()
        status.empty()
        ...
        st.dataframe(all_df, use_container_width=True, height=520, hide_index=True)

        # Portfolio totals
        inv = all_df["Invested"].sum()
        book = all_df["Book value"].sum()
        c1, c2, c3 = st.columns(3)
        c1.metric("Total invested", f"₹{inv:,.0f}")
        c2.metric("Book value", f"₹{book:,.0f}")
        c3.metric("Book P&L", f"₹{book-inv:,.0f}", f"{((book-inv)/inv)*100:+.1f}%")
        if fetch_live_all and "Live Value" in all_df.columns:
            live_sum = all_df["Live Value"].dropna().sum()
            inv_live = all_df.loc[all_df["Live Value"].notna(), "Invested"].sum()
            if inv_live > 0:
                st.metric("Live P&L (fetched names)", f"₹{live_sum - inv_live:,.0f}", f"{((live_sum-inv_live)/inv_live)*100:+.1f}%")
        ...
        # Batch PDF (compact table-style pages)
        if st.button("Generate PDF for all 52 (summary)", type="primary", key="pdf_all52"):
            def pdf_safe(s):
                if s is None:
                    return "-"
                s = str(s)
            ...
            try:
                ...
                pdf.cell(0, 8, pdf_safe(f"Portfolio Summary - {datetime.now().strftime('%d %b %Y')}"), new_x="LMARGIN", new_y="NEXT")
                pdf.set_font("Helvetica", "", 9)
                pdf.cell(0, 6, pdf_safe(f"Invested Rs {inv:,.0f} | Book Rs {book:,.0f} | P&L {((book-inv)/inv)*100:+.1f}%"), new_x="LMARGIN", new_y="NEXT")
                pdf.ln(2)
                # header
                cols = ["Symbol", "Sector", "Frame", "Action", "Invested", "Avg", "LTP", "P&L%", "Wt%"]
                widths = [28, 45, 22, 28, 28, 22, 22, 18, 16]
                ...
                for _, row in all_df.iterrows():
                    ...
                    for v, w in zip(vals, widths):
                        pdf.cell(w, 5, pdf_safe(v), border=1)
                    pdf.ln()
                    if pdf.get_y() > 190:
                        pdf.add_page()
                        pdf.set_font("Helvetica", "B", 8)
                ...
                pdf.multi_cell(0, 5, "Educational only. Not investment advice.")
                buf = __import__("io").BytesIO()
                pdf.output(buf)
                st.download_button(
                    "Download all-52 PDF",
                    data=buf.getvalue(),
                    file_name=f"portfolio_all52_{datetime.now().strftime('%Y%m%d')}.pdf",
                    mime="application/pdf",
                    key="dl_all52_pdf",
                )
                st.success("All-52 PDF ready.")
            except Exception as e:
                st.error(f"PDF failed: {e}")

        st.caption("Switch to Single stock for full deep-dive (highs/lows chart, Yahoo fundamentals, promoter notes, sector write-up).")
        st.stop()
    ...
    if st.button("Generate PDF report", type="primary"):
        def pdf_safe(s):
            ...
            for a, b in repl.items():
                s = s.replace(a, b)
            # Keep only latin-1 safe chars for Helvetica
            return "".join(ch if ord(ch) < 256 else "?" for ch in s)

        def pdf_line(pdf, txt, size=10, bold=False):
            ...
            if not safe.strip():
                safe = "-"
            # multi_cell can fail if width is tiny; force minimum
            if w < 20:
                w = 180
        ...
        try:
            ...
            pdf.add_page()
            pdf_line(pdf, f"Stock Report: {sym}", size=16, bold=True)
            pdf_line(pdf, f"Generated: {datetime.now().strftime('%d %b %Y %H:%M')} IST", size=11)
            pdf_line(pdf, f"Sector: {r.sector} | Framework: {r.frame} | Action: {r.action}", size=11)
            pdf.ln(3)
            pdf_line(pdf, "1. Position", size=12, bold=True)
            pdf_line(pdf, f"Invested: Rs {r.invested:,.0f} | Avg: Rs {r.avg} | Book LTP: Rs {r.ltp}", size=10)
            pdf_line(pdf, f"Qty: {r.qty} (LT {r.lt} / ST {r.st}) | Book value: Rs {r.current:,.0f}", size=10)
            if live and live.get("price"):
                lp = live["price"]
                live_pnl = lp * r.qty - r.invested
                pdf_line(pdf, f"Live: Rs {lp:,.2f} | Live P&L: Rs {live_pnl:,.0f} ({(live_pnl/r.invested)*100:+.1f}%)", size=10)
            pdf.ln(2)
            pdf_line(pdf, "2. High / Low / Avg", size=12, bold=True)
            for row in rows_hl:
                if row.get("high"):
                    line = f"{row['period']}: High Rs {row['high']:,.1f} | Low Rs {row['low']:,.1f} | Avg Rs {row['avg']:,.1f} | From high {row['from_high_pct']:+.1f}%"
                    pdf_line(pdf, line, size=10)
            pdf.ln(2)
            pdf_line(pdf, "3. Business & Research", size=12, bold=True)
            pdf_line(pdf, f"Signal: {r.signal}", size=10)
            pdf_line(pdf, f"Note: {r.note or '-'}", size=10)
            pdf_line(pdf, f"Research: {research.get(sym, 'No extended note.')}", size=10)
            pdf.ln(2)
            pdf_line(pdf, "4. Promoter / Institutional", size=12, bold=True)
            pdf_line(pdf, promoter_notes.get(sym, "Check NSE shareholding pattern for live promoter/FII/DII %."), size=10)
            pdf.ln(2)
            pdf_line(pdf, "5. Sector", size=12, bold=True)
            pdf_line(pdf, (sector_ctx.get(r.sector, "-") or "-")[:500], size=10)
            pdf.ln(3)
            pdf_line(pdf, "Disclaimer", size=11, bold=True)
            pdf_line(pdf, "Educational only. Not investment advice. Live data from Yahoo Finance may be delayed. Verify on NSE/BSE. Consult SEBI-registered advisor.", size=9)
            buf = io.BytesIO()
            pdf.output(buf)
            st.download_button(
                "Download PDF",
            ...
            st.success("PDF ready — use the Download button above.")
        except Exception as e:
            st.error(f"PDF generation failed: {e}")
            st.info("You can still copy the on-screen report. Try another stock or refresh the page.")
